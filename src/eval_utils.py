    ...
    for s in sents:
        try:
            ...
            try:
                ...
                pass
            ac, at, sp, ot = '', '', '', ''
        if (ac, at, sp, ot) not in quads:
            quads.append((ac, at, sp, ot))

    return quads

def compute_f1_scores(pred_pt, gold_pt, num, verbose=True):
    """
    ...
    if verbose:
        print(
            f"num: {num} number of gold spans: {n_gold}, predicted spans: {n_pred}, hit: {n_tp}"
        )
...
def compute_scores(pred_seqs, gold_seqs, verbose=True):
    ...
    for i in range(num_samples):
        ...
        all_labels[0].append(gold_list)
        all_preds[0].append(pred_list)
        if len(gold_list) == 1:
            all_labels[1].append(gold_list)
            all_preds[1].append(pred_list)
        elif len(gold_list) == 2:
            all_labels[2].append(gold_list)
            all_preds[2].append(pred_list)
        else:
            all_labels[3].append(gold_list)
            all_preds[3].append(pred_list)
    
    scores = []
    for i in range(4):
        scores.append(compute_f1_scores(all_preds[i], all_labels[i], num=i))
